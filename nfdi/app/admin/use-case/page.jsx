...
const PPManagerPage = () => {
    ...
    useEffect(() => {
        const fetchUC = async () => {
            const res = await fetch('/api/use-case/all')
            if (res.ok) {
    ...
    const onDeleted = (deleted) => {
        if (deleted) {
            ...
            const newUCs = UCs.filter((e) => e.id != id)
            setUCs(newUCs)
        }
    }
    ...
    return (
        <div className="w-full">
            <div className="flex justify-between">
                <h2 className="font-bold text-xl">Quản lý trường hợp sử dụng</h2>
                <button className="dark:bg-bodydark dark:text-black-2 text-black bg-white rounded-md px-3 py-2 shadow-md"><Link href={"/admin/use-case/create"}>Thêm THSD</Link></button>
            </div>
            {!loading ? (
                ...
                <table className="w-full text-sm text-center text-gray-500 dark:text-white">
                    ...
                    <tbody>
                        {UCs.map((pp, index) => (
                            <tr key={pp.id} className={`bg-white ${index == UCs.length - 1 ? "" : "border-b"} dark:bg-bodydark text-black border-body`}>
                                ...
                                <td className="px-6 py-4">
                                    <Link href={"/admin/use-case/update/" + pp.id} className="hover:text-boxdark-2 hover:font-bold">Xem chi tiết</Link>
                                </td>
                                <td className="px-6 py-4">
                                    <button onClick={() => {setOpenModal(!openModal); setDeleteLink("/api/use-case/" + pp.id)}} className="hover:text-boxdark-2 hover:font-bold">Xóa</button>
                                </td>
                            </tr>
