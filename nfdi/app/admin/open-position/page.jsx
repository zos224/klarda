...
const linkPage = () => {
    ...
    useEffect(() => {
        const fetchlink = async () => {
            const res = await fetch('/api/open-position/all')
            if (res.ok) {
    ...
    const onDeleted = (deleted) => {
        if (deleted) {
            ...
            const newopenPositions = openPositions.filter((e) => e.id != id)
    ...
    return (
        <div className="w-full">
            <div className="flex justify-between">
                <h2 className="font-bold text-xl">Quản lý tuyển dụng</h2>
                <button className="dark:bg-bodydark dark:text-black-2 text-black bg-white rounded-md px-3 py-2 shadow-md"><Link href={"/admin/open-position/create"}>Thêm công việc</Link></button>
            </div>
            {!loading ? (
                ...
                <table className="w-full text-sm text-center text-gray-500 dark:text-white">
                    ...
                    <tbody>
                        {openPositions.map((op, index) => (
                            <tr key={op.id} className={`bg-white ${index == openPositions.length - 1 ? "" : "border-b"} dark:bg-bodydark text-black border-body`}>
                                ...
                                <td className="px-6 py-4">
                                    <Link className="hover:text-boxdark-2 hover:font-bold" href={"/admin/open-position/update/" + op.id}>Cập nhật</Link>
                                </td>
                                <td className="px-6 py-4">
                                    <button onClick={() => {setOpenModal(!openModal); setDeleteLink("/api/open-position/" + op.id)}} className="hover:text-boxdark-2 hover:font-bold">Xóa</button>
                                </td>
                            </tr>
