...
const EventPage = () => {
    ...
    useEffect(() => {
        const fetchTopics = async () => {
            const res = await fetch('/api/topic/get')
            if (res.ok) {
    ...
    return (
        <div className="w-full">
            <div className="flex justify-between mx-4">
                ...
                <button className="dark:bg-bodydark dark:text-black-2 text-black bg-white rounded-md px-3 py-2 shadow-md" onClick={() => {setCurrentTopic({id: null, name: ''}); setAction("create"); setOpenModalTopic(true)}}>Thêm chủ đề</button>
            </div>
            {!loading ? (
                ...
                <table className="w-full text-sm text-center text-gray-500 dark:text-white">
                    ...
                    <tbody>
                        {topics.map((topic, index) => (
                            <tr key={topic.id} className={`bg-white ${index == topics.length - 1 ? "" : "border-b"} dark:bg-bodydark text-black border-body`}>
                ...
                <ModalParent action={action} data={currentTopic} isOpen={openModalTopic} name={"topic"} onClose={(topic) => {setOpenModalTopic(false); if (topic) updateOrInsert(topic)}} vie={"Chủ đề"}></ModalParent>
            </div>
            ) : (
