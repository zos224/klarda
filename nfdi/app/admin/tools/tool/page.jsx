...
const toolPage = () => {
    ...
    useEffect(() => {
        const fetchtool = async () => {
            const res = await fetch('/api/tool/all')
            if (res.ok) {
    ...
    const onDeleted = (deleted) => {
        if (deleted) {
            ...
            const newtools = tools.filter((e) => e.id != id)
            setTools(newtools)
        }
    }
    ...
    return (
        <div className="w-full">
            <div className="flex justify-between">
                <h2 className="font-bold text-xl">Quản lý công cụ</h2>
                <button className="dark:bg-bodydark dark:text-black-2 text-black bg-white rounded-md px-3 py-2 shadow-md"><Link href={"/admin/tools/tool/create"}>Thêm công cụ</Link></button>
            </div>
            {!loading ? (
                ...
                <table className="w-full text-sm text-center text-gray-500 dark:text-white">
                    ...
                    <tbody>
                        {tools.map((tool, index) => (
                            <tr key={tool.id} className={`bg-white ${index == tools.length - 1 ? "" : "border-b"} dark:bg-bodydark text-black border-body`}>
                                ...
                                <td className="px-6 py-4">
                                    <Link className="hover:text-boxdark-2 hover:font-bold" href={"/admin/tools/tool/update/" + tool.id}>Cập nhật</Link>
                                </td>
                                <td className="px-6 py-4">
                                    <button onClick={() => {setOpenModal(!openModal); setDeleteLink("/api/tool/" + tool.id)}} className="hover:text-boxdark-2 hover:font-bold">Xóa</button>
                                </td>
                            </tr>
